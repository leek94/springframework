...
public class Ch07Controller {
	...
	public ModelAndView request1(Model model) {
		...
		for (int i = 1; i <= 10; i++) {
			boardList.add(new Ch07Board(i, "제목" + i, "봄날" + i, "글쓴이" + i, 0, new Date()));
		}
	...
	public String request2(Model model, Ch07Board board) {
		...
		joinForm.setMid("KYJ");
		joinForm.setMname("김영주");
		joinForm.setMpassword("84763420");
		joinForm.setMjob("공무원");
		joinForm.setMcity("서울");
	...
	public String postRequest2(Model model, @ModelAttribute("joinForm") Ch07JoinForm joinForm) {
		model.addAttribute("chNum", "ch07");
		// model.addAttribute("ch07JoinForm", joinForm); 기본적으로 앞글자 소문자로 기능, 어노테이션 붙여서 지정해도 됨
	...
	public String sessionLoginOut(Model model, HttpSession session) {
